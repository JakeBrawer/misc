  ...
  while(pass_counter < size){ 
    ...
    if(pass_counter < size-1){
      ...
      if(rank%2 ==0){
        ...
        // send to col to next proc
        MPI_Send(Brecv,BsendCount[col_rank],MPI_DOUBLE,(rank+1)%size,1,MPI_COMM_WORLD);
        free(Brecv); // next col may not be the same size as current so free space.
        ...
        tmp_Brecv = (double *)calloc(BsendCount[col_rank],sizeof(double));
        MPI_Recv(tmp_Brecv,BsendCount[col_rank],MPI_DOUBLE,MOD(rank - 1,size),1,MPI_COMM_WORLD,&status);


        MPI_Send(Brecv,BsendCount[(col_rank+1)%size],MPI_DOUBLE,(rank+1)%size,1,MPI_COMM_WORLD);
        free(Brecv);
