...
int main(int argc, char *argv[]){
  ...
  if(argc < 5|| argc > 5){
    printf("Usage: island <n> <b> <pop_size> <max gens>\n");
    exit(-1);
  }
  ...
  srand(12345 + rank); // each processes gets its own seed
  ...
  while(gen_counter < max_gens && global_stop < 0){
    double sendbuff[n];
    double recvbuff[n * size];
    double recvfitnesses[size];
    generation o;
    ...
    if(gen_counter % 10  == 0){
      ...
      // send most fit agent to all procs
      MPI_Allgather(sendbuff, n,MPI_DOUBLE,recvbuff,n, MPI_DOUBLE,MPI_COMM_WORLD);
      MPI_Allgather(&max_fitness, 1,MPI_DOUBLE,recvfitnesses,1, MPI_DOUBLE,MPI_COMM_WORLD);
      ...
      for(int k = 0; k < size; k++){
        ...
        for (int i = 0; i < gen_size; i++) {
          ...
          if(p.fitnesses[i] < recvfitnesses[k] && k != rank){

            for(int j = 0; j < n; j++){
              p.agents[i * n + j] = recvbuff[k * n + j];
            }
            p.fitnesses[i] = recvfitnesses[k];
            break;
          }
        }
      }
