...
__global__ void tiled_matrixmult(FP *a,FP *b, FP *c,
  ...
  // This loop shifts the tiles across each matrix
  for(int i = 0; i < p /(float)  tileDim; i++ ){
    
    int indexA = i*tileDim + blockIdx.y*tileDim*p + threadIdx.y*p + threadIdx.x;

    // We update Asub here so that we can keep it in shared memory longer
    ...
    // cycles through adjacent tiles
    for(int adj = 0; adj < adj_tiles; adj++){
      int widthOverflowA = (i+1)*tileDim;

      // where are the submatrices in the orignal matricies?
      int indexB = (blockIdx.x*adj_tiles + adj)*tileDim + i*tileDim*m + threadIdx.y*m + threadIdx.x;

      int indexC = (blockIdx.x*adj_tiles + adj) * tileDim + blockIdx.y * tileDim * m + threadIdx.y*m + threadIdx.x;
      ...
      if(col < m && row < n){
        ...
        if( i*tileDim + threadIdx.y < p){
          ...
          /*   DEBUG_PRINT("i: %d adj %d: \n",i,adj); */
          /*   DEBUG_PRINT("index: %d\n", index); */
          /*   DEBUG_PRINT("row: %d col: %d\n",row,col); */
          /*   DEBUG_PRINT("indexB: %d \n",indexB); */
...
int main(int argc, char *argv[]) {
  ...
  int Grid_X = 1; //Grid dimension, x and y
  int Grid_Y = 1;
  ...
  if ((argc<8) || (argc>9)) {
    printf("Usage: matmul  <n> <p> <m> <block dim> <grid dim> [<dev num>] <num adj tiles>\n");
    exit (-1);
  }
  ...
  tiled_matrixmult<<<Grid,Block,tileDim*sizeof(FP)>>>(dev_a,dev_b,dev_c,n,p,m,Block_Dim,adj_tiles);
