...
void MoveBodies(const int nBodies,  BodySet* const bodies,
 ...
 //Step through bodies by TILE
  #pragma omp parallel for 
  for (int i = 0; i < nBodies-TILE; i+=TILE) { 
    // Components of the gravity force on body i
    float Fx = 0, Fy = 0, Fz = 0; 
      // where main tiling is happening
      for (int j = 0; j < nBodies; j++) { 
         #pragma unroll_and_jam(TILE)
          for(int x = i; x < i+TILE; x++){
            // Newton's law of universal gravity
            const float dx = bodies->x[j]- bodies->x[x];
            const float dy = bodies->y[j]- bodies->y[x];
            const float dz = bodies->z[j]- bodies->z[x];

            const float drSquared  = dx*dx + dy*dy + dz*dz;
	    // here is fast inverse sqrt
